...
def exer():
    for i in data:
        if(len(i['mechanics'])>0):
            ...
            g.db.execute('insert into exercise (mechanics,difficulty,detailed_muscle,exercise_name,equipments,other_muscle_group,muscle,type,procedure) values (?,?,?,?,?,?,?,?,?)',[mechanics,difficulty,detailed_muscle,name,equipments,group,muscle,type,procedure])
            g.db.commit()
    return 'yo'
